...
export class AriaPaperLibraryMcpServer {
	...
	async start(): Promise<number> {
		...
		for (const candidate of candidates) {
			try {
				...
				console.log(`[aria-paper-search] MCP server listening on http://${HOST}:${this.port}`);
				return this.port;
			} catch (err) {
				...
				console.warn(`[aria-paper-search] port ${candidate} in use, trying next…`);
			}
		}
		throw new Error('Could not find a free port for the paper-library MCP server');
	}
	...
	async stop(): Promise<void> {
		...
		console.log('[aria-paper-search] MCP server stopped');
	}
