...
export class AriaNotesMcpServer {
	...
	async start(): Promise<number> {
		...
		for (const candidate of candidates) {
			try {
				...
				console.log(`[aria-notes] MCP server listening on http://${HOST}:${this.port}`);
				return this.port;
			} catch (e) {
				...
				console.warn(`[aria-notes] port ${candidate} in use, trying next…`);
			}
		}
	...
	async stop(): Promise<void> {
		...
		console.log('[aria-notes] MCP server stopped');
	}
	...
	private handle(req: http.IncomingMessage, res: http.ServerResponse): void {
		...
		if (req.method === 'GET' && url.pathname === '/sse') {
			...
			res.end(JSON.stringify({ server: 'aria-notes', toolCount: this.tools.length }));
		} else {
			res.writeHead(404);
			res.end();
		}
