...
export async function registerWithClaudeCode(port: number): Promise<RegistrationResult> {
	console.log(`[aria-notes] registerWithClaudeCode(port=${port}) starting`);
	const claude = await resolveClaudeBinary();
	if (!claude) {
		console.error('[aria-notes] Claude CLI not found in PATH or candidate locations');
		return { ok: false, changed: false, message: 'Claude CLI not found on PATH or known install locations.' };
	}
	console.log(`[aria-notes] Claude CLI resolved to: ${claude}`);
	...
	if (existingPort === port) {
		console.log(`[aria-notes] Claude Code already registered on port ${port}; skipping re-registration`);
	...
	for (const scope of ['user', 'project', 'local']) {
		try {
			...
			console.log(`[aria-notes] removed prior MCP entry --scope ${scope}:`, out.stdout.trim());
		} catch (e) {
			console.log(`[aria-notes] no prior MCP entry --scope ${scope}:`, (e as Error).message);
		}
	}

	const addCmd = `${q} mcp add --scope user ${MCP_NAME} ${quoteArg(url)} --transport sse`;
	console.log(`[aria-notes] running: ${addCmd}`);
	const runAdd = async (): Promise<{ ok: boolean; stderr: string }> => {
		try {
			const out = await execAsync(addCmd, { timeout: 10000 });
			console.log(`[aria-notes] mcp add stdout:`, out.stdout.trim());
			console.log(`[aria-notes] mcp add stderr:`, out.stderr.trim());
	...
	if (!addResult.ok && /already exists/i.test(addResult.stderr)) {
		// Race condition: autopipe / paper-search / aria-notes all touch
		...
		console.warn('[aria-notes] mcp add raced with another writer; retrying after re-remove');
		try {
			await execAsync(`${q} mcp remove ${MCP_NAME} --scope user`, { timeout: 10000 });
		} catch (e) {
			console.log(`[aria-notes] retry-remove failed (likely already gone):`, (e as Error).message);
		}
		await new Promise(r => setTimeout(r, 500));
		addResult = await runAdd();
	}
	if (!addResult.ok) {
		console.error(`[aria-notes] claude mcp add failed:`, addResult.stderr);
	...
	try {
		...
		console.warn(`[aria-notes] claude mcp list verification failed:`, e);
	}
