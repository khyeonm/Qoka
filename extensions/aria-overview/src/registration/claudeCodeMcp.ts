 ...
 * list used by aria-autopipe so behaviour stays consistent - including
 * the nvm-managed node bins, which is how the Claude CLI is actually
 * reachable on this user's box.
 */
...
/**
 * (Re-)register the Qoka Roadmap MCP server with Claude Code. Mirrors the
 * autopipe pattern: always best-effort removes any prior entry across all
 * three scopes, then adds the current live port at --scope user so the MCP
 * is reachable from every project Claude Code opens.
 */
export async function registerWithClaudeCode(port: number): Promise<RegistrationResult> {
	console.log(`[aria-overview] registerWithClaudeCode(port=${port}) starting`);
	const claude = await resolveClaudeBinary();
	if (!claude) {
		console.error('[aria-overview] Claude CLI not found in PATH or candidate locations');
		return { ok: false, changed: false, message: 'Claude CLI not found on PATH or known install locations.' };
	}
	console.log(`[aria-overview] Claude CLI resolved to: ${claude}`);
	...
	if (existingPort === port) {
		console.log(`[aria-overview] Claude Code already registered on port ${port}; skipping re-registration`);
	...
	for (const scope of ['user', 'project', 'local']) {
		try {
			...
			console.log(`[aria-overview] removed prior MCP entry --scope ${scope}:`, out.stdout.trim());
		} catch (e) {
			console.log(`[aria-overview] no prior MCP entry --scope ${scope}:`, (e as Error).message);
		}
	}

	const addCmd = `${q} mcp add --scope user ${MCP_NAME} ${quoteArg(url)} --transport sse`;
	console.log(`[aria-overview] running: ${addCmd}`);
	const runAdd = async (): Promise<{ ok: boolean; stderr: string }> => {
		try {
			const out = await execAsync(addCmd, { timeout: 10000 });
			console.log(`[aria-overview] mcp add stdout:`, out.stdout.trim());
			console.log(`[aria-overview] mcp add stderr:`, out.stderr.trim());
	...
	if (!addResult.ok && /already exists/i.test(addResult.stderr)) {
		// Race condition: autopipe / paper-search / aria-overview all touch
		// the same `~/.claude.json` from concurrent CLI invocations. Our
		// earlier remove may have been overwritten by another extension's
		// in-flight write, leaving the stale entry behind. Re-run the
		// remove after a short jitter and try the add once more.
		console.warn('[aria-overview] mcp add raced with another writer; retrying after re-remove');
		try {
			await execAsync(`${q} mcp remove ${MCP_NAME} --scope user`, { timeout: 10000 });
		} catch (e) {
			console.log(`[aria-overview] retry-remove failed (likely already gone):`, (e as Error).message);
		}
		await new Promise(r => setTimeout(r, 500));
		addResult = await runAdd();
	}
	if (!addResult.ok) {
		console.error(`[aria-overview] claude mcp add failed:`, addResult.stderr);
	...
	try {
		...
		console.warn(`[aria-overview] claude mcp list verification failed:`, e);
	}
