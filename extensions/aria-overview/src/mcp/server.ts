...
const SERVER_INSTRUCTIONS = [
	...
	'   confirm it reads correctly.',
	'2. After they confirm, say you will plan how to proceed, and call open_roadmap with a short descriptive',
	'   `title` (this switches to the Roadmap tab and opens THIS project\'s roadmap canvas - a new project',
	'   already has exactly one empty roadmap, so it opens that one, named by your title instead of',
	'   "Untitled roadmap"; it does NOT create a duplicate).',
	...
	'5. If they agree, move straight on to the To-do: draft it with add_tasks - ACTION-oriented items the',
	'   user will actually DO (experiments / analyses / concrete steps), NOT a 1:1 copy of the roadmap.',
	'   Then call open_overview and tell the user WHERE things are: the roadmap is on the Overview tab and the',
	'   To-do list is placed BELOW it. A large roadmap can push the list off-screen, so explicitly say the',
	'   To-do list is written just below the roadmap (scroll down under the roadmap to see it). They can',
	'   review and edit both freely.',
	'6. Then offer the next step as an OPEN CHOICE - do NOT assume autopipe. Present a few options that fit',
	'   this project and let the user pick, e.g. "Would you like to look for related papers, or build a',
	'   runnable pipeline with autopipe?" (add other fitting options as relevant). Proceed with autopipe ONLY',
	'   if the user chooses it; if they pick papers, use the paper-search tools; otherwise follow their pick.',
	'',
