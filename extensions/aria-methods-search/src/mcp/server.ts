 ...
 * coexist with the other Aria MCP servers when all extensions are active.
 */
export class AriaMethodsSearchMcpServer {
	...
	async start(): Promise<number> {
		...
		for (const candidate of candidates) {
			try {
				...
				console.log(`[aria-methods-search] MCP server listening on http://${HOST}:${this.port}`);
				return this.port;
			} catch (err) {
				...
				console.warn(`[aria-methods-search] port ${candidate} in use, trying next…`);
			}
		}
		throw new Error('Could not find a free port for the methods-search MCP server');
	}
	...
	async stop(): Promise<void> {
		...
		console.log('[aria-methods-search] MCP server stopped');
	}
