...
async function resolveClaude(): Promise<string | null> {
	...
	for (const candidate of CLAUDE_CANDIDATES) {
		try {
			...
			console.log(`[aria-methods-search] resolveClaude -> ${candidate} (candidate)`);
			return candidate;
		} catch { /* try next */ }
	}
	console.warn('[aria-methods-search] resolveClaude -> null (claude CLI not found)');
	return null;
}
...
/**
 * Read methods-search's port from ~/.claude.json's USER-scope mcpServers
 * (the top-level `mcpServers` object, not any per-project block). Returns null
...
function readUserScopeRegisteredPort(): number | null {
	...
	try {
		...
		console.error(`[aria-methods-search] readUserScopeRegisteredPort error:`, (err as Error).message);
		return null;
	}
}

/**
 * Register the methods-search MCP server with the Claude Code CLI. Uses the
 * HTTP+SSE transport endpoint (/sse), distinct from Codex's Streamable HTTP
 * endpoint at /mcp.
 */
export async function registerWithClaudeCode(port: number): Promise<RegistrationResult> {
	console.log(`[aria-methods-search] registerWithClaudeCode(port=${port}) starting`);
	const claude = await resolveClaude();
	if (!claude) {
		return {
			ok: false, changed: false,
			message: 'Claude CLI not found. Install the Claude Code extension to register methods-search.',
		};
	}
	...
	try {
		...
		console.error(`[aria-methods-search] claude mcp add failed: ${stderr.trim()}`);
