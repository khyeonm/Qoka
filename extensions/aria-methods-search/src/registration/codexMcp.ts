...
/**
 * Register methods-search with the Codex CLI. Codex uses Streamable HTTP
 * (MCP protocol 2025-03-26+) at /mcp - different transport from Claude Code's
 * /sse.
 */
export async function registerWithCodex(port: number): Promise<RegistrationResult> {
	...
	// Skip work when Codex is already pointed at our live port. Codex uses a
	// single user-scope config (config.toml), so reading `mcp list` is enough.
