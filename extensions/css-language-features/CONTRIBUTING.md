...
- Open `/extensions/css-language-features/` as the workspace in Qoka
...
- Run the [`Launch Extension`](https://github.com/microsoft/vscode/blob/master/extensions/css-language-features/.vscode/launch.json) debug target in the Debug View. This will:
	- Launch a new Qoka instance with the `css-language-features` extension loaded
...
- Debug the language server process by using `Attach to Node Process` command in the  Qoka window opened on `css-language-features`.
