...
/**
 * Register qoka-slides with the Codex CLI. Codex uses Streamable HTTP at /mcp -
 * different transport from Claude Code's /sse. The server serves both endpoints.
 */
export async function registerWithCodex(port: number): Promise<RegistrationResult> {
	...
	// ~/.codex/config.toml. On Windows they collide on the file lock; retry with
	// jittered backoff so contending writers each succeed in turn.
	let addErr = '';
