...
const QOKA_BLOCK = [
	...
	'  (autopipe MCP) first to confirm the active run connection, then `run_code` (qoka-run MCP) for a quick',
	'  script, or `execute_pipeline` (autopipe MCP) for a reproducible pipeline.',
	...
	'- SAVE a paper to the library / list saved papers -> `save_paper` / `list_saved_papers` (paper-library MCP).',
	'- Plan a project / build a roadmap -> the roadmap MCP tools.',
	'- Take or organise research notes -> the notes MCP tools.',
	'- Search methods or hypotheses -> the `methods-search` / `hypothesis` MCP tools.',
