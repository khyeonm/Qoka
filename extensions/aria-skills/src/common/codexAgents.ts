...
const QOKA_BLOCK = [
	...
	'- ASK FIRST BY DEFAULT: when the user just says "run/execute this code" (실행/돌려) without saying whether',
	'  they want a quick run or a reproducible pipeline, ASK before running: "간단하게 바로 실행할까요(run_code),',
	'  아니면 autopipe 파이프라인으로 만들까요?". Do NOT default to a pipeline - it needs a Snakefile/config/Dockerfile',
	'  and is much heavier than run_code, and having used autopipe before is NOT a reason to assume a pipeline now.',
	'  Only skip the question when the user was explicit (e.g. "그냥 빨리 돌려줘" -> run_code; "파이프라인으로 만들어줘" -> execute_pipeline).',
