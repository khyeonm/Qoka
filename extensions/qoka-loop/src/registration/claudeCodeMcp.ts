...
export async function registerWithClaudeCode(port: number): Promise<RegistrationResult> {
	console.log(`[qoka-loop] registerWithClaudeCode(port=${port}) starting`);
	const claude = await resolveClaudeBinary();
	if (!claude) {
		console.error('[qoka-loop] Claude CLI not found in PATH or candidate locations');
		return { ok: false, changed: false, message: 'Claude CLI not found on PATH or known install locations.' };
	}
	console.log(`[qoka-loop] Claude CLI resolved to: ${claude}`);
	...
	if (existingPort === port) {
		console.log(`[qoka-loop] Claude Code already registered on port ${port}; skipping re-registration`);
	...
	for (const scope of ['user', 'project', 'local']) {
		try {
			...
			console.log(`[qoka-loop] removed prior MCP entry --scope ${scope}:`, out.stdout.trim());
		} catch (e) {
			console.log(`[qoka-loop] no prior MCP entry --scope ${scope}:`, (e as Error).message);
		}
	}

	const addCmd = `${q} mcp add --scope ${scope} ${MCP_NAME} ${quoteArg(url)} --transport sse`;
	console.log(`[qoka-loop] running: ${addCmd}`);
	const runAdd = async (): Promise<{ ok: boolean; stderr: string }> => {
		try {
			const out = await execAsync(addCmd, opts);
			console.log(`[qoka-loop] mcp add stdout:`, out.stdout.trim());
			console.log(`[qoka-loop] mcp add stderr:`, out.stderr.trim());
	...
	if (!addResult.ok && /already exists/i.test(addResult.stderr)) {
		// Race condition: autopipe / paper-search / qoka-loop all touch
		...
		console.warn('[qoka-loop] mcp add raced with another writer; retrying after re-remove');
		try {
			await execAsync(`${q} mcp remove ${MCP_NAME} --scope ${scope}`, opts);
		} catch (e) {
			console.log(`[qoka-loop] retry-remove failed (likely already gone):`, (e as Error).message);
		}
		await new Promise(r => setTimeout(r, 500));
		addResult = await runAdd();
	}
	if (!addResult.ok) {
		console.error(`[qoka-loop] claude mcp add failed:`, addResult.stderr);
	...
	try {
		...
		console.warn(`[qoka-loop] claude mcp list verification failed:`, e);
	}
