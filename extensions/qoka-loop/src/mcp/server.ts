...
const SERVER_INSTRUCTIONS = [
	...
	'',
	'== loop vs run_code: how to decide (important) ==',
	'Route the request BEFORE doing the work:',
	' - Run it as a LOOP (call design_loop) when the request looks like REPEATED work toward a goal / threshold /',
	'   condition (in any language - e.g. "keep trying until Y passes", "improve X until the mean is 50", "generate',
	'   candidates and keep the ones that dock well"), OR when the word "loop" appears ("make it a loop", "as a loop").',
	' - Run it as a one-shot with run_code when it is a single task that finishes in one pass with no repeat/threshold.',
	' - If it is AMBIGUOUS (could reasonably be either), ASK the user (in their own language) whether to run it once',
	'   with run_code or as a repeating loop, and wait for their answer. Do NOT guess.',
	'Never force a loop onto an ordinary one-shot task, and never silently do repeated work by hand that should be a loop.',
	'',
	'== How to run a loop ==',
	...
	'   design_loop\'s rules also tell you what you MUST re-ask the user before building (stop condition, required',
	'   output files, budget) - follow them; do not invent unstated requirements.',
	'2. Show the user a SHORT plan summary (goal, how the evaluator decides done, the required output files, and the',
	'   budget = max iterations + max minutes per iteration) and ask, in their own language, "is this the loop flow',
	'   you want?". Note it runs sub-agents in the background and consumes their tokens. NEVER run without approval.',
	'3. On approval: save_loop(spec) -> start_loop(loopId). Do NOT poll; the loop runs in the background and a',
	'   corner notification fires when it finishes. RIGHT AFTER start_loop returns, tell the user (in their own',
	'   language) that the loop has started and that they can watch it live in the loop\'s detail tab: a "Progress"',
	'   section appears there UNDER the Flow, showing the current iteration and step. This section only exists',
	'   while the loop is running, so point the user to it now.',
	'',
	'== Controlling loops ==',
