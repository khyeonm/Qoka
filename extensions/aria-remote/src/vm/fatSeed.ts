...
//
// COPY of extensions/aria-autopipe/src/vm/fatSeed.ts (pure, no deps) — see
// provisioner.ts here for why aria-remote carries its own copy.
//
...
/** 8.3 short-name alias (11 bytes: 8 name + 3 ext, space-padded). */
...
/** Build a raw FAT12 disk image (label `cidata`) holding the given files. */
export function buildFatSeedImage(files: SeedFile[], volumeLabel = 'cidata'): Buffer {
	...
	img.writeUInt32LE(0, 32);  // total sectors (32-bit)
	img[36] = 0x00; // drive number
	img[38] = 0x29; // extended boot signature
	img.writeUInt32LE(0x41524941, 39); // volume id ("ARIA")
