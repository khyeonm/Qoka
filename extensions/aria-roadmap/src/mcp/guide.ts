...
export const ROADMAP_BRAINSTORM_GUIDE = [
	...
	'- The hand-off is two steps, in order: (1) call `add_tasks` (the qoka-overview tool) to break the plan into research / work-unit ACTION items the user will actually DO (experiments, analyses, concrete steps - NOT a 1:1 copy of the nodes), then (2) call `open_overview` (also qoka-overview) so the user lands on the Notebook tab.',
	'- After `open_overview`, tell the user WHERE things are: the roadmap is now shown on the Notebook tab and the To-do list is placed BELOW it (a large roadmap can push the list off-screen, so say to scroll down under the roadmap to see it). They can review and edit both freely.',
