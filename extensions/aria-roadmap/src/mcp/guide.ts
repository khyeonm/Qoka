...
export const ROADMAP_BRAINSTORM_GUIDE = [
	...
	'- Before you propose ANY nodes, make sure the roadmap canvas is OPEN and visible so the user can actually watch them appear. If it is not already open, call `open_roadmap` (the qoka-overview tool) FIRST, then propose. NEVER tell the user you added nodes while the canvas is closed - they would have nothing to look at.',
	...
	'',
	'Saving:',
	'- The roadmap saves AUTOMATICALLY on every edit - there is NO Save button and nothing for the user to press.',
	'- NEVER tell the user to save, to press a Save button, or to click anything to keep the roadmap. If they ask about saving, reassure them their changes are already saved automatically as they go.',
	'',
	'',
	'Completing the roadmap (mandatory hand-off):',
	'- When the roadmap is complete - including when the user says something like "let\'s finish the roadmap here" / "this looks done" - you MUST complete the hand-off below FIRST so the plan actually moves onto the Overview and To-do. Do the hand-off, THEN still offer the open next-step choice as usual.',
	'- The hand-off is two steps, in order: (1) call `add_tasks` (the qoka-overview tool) to break the plan into research / work-unit ACTION items the user will actually DO (experiments, analyses, concrete steps - NOT a 1:1 copy of the nodes), then (2) call `open_overview` (also qoka-overview) so the user lands on the Project Overview tab.',
	'- After `open_overview`, tell the user WHERE things are: the roadmap is now shown on the Overview tab and the To-do list is placed BELOW it (a large roadmap can push the list off-screen, so say to scroll down under the roadmap to see it). They can review and edit both freely.',
	'- You MAY still call `finalize_roadmap` as an optional completion marker, but it does NOT replace the hand-off - add_tasks + open_overview are what actually move the plan onto the Overview and To-do.',
	'',
	'',
