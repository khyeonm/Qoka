...
export const ROADMAP_BRAINSTORM_GUIDE = [
	...
	'- Before you propose ANY nodes, make sure the roadmap canvas is OPEN and visible so the user can actually watch them appear. If it is not already open, call `open_roadmap` (the aria-overview tool) FIRST, then propose. NEVER tell the user you added nodes while the canvas is closed - they would have nothing to look at.',
	...
	'- The hand-off is two steps, in order: (1) call `add_tasks` (the aria-overview tool) to break the plan into research / work-unit ACTION items the user will actually DO (experiments, analyses, concrete steps - NOT a 1:1 copy of the nodes), then (2) call `open_overview` (also aria-overview) so the user lands on the Project Overview tab.',
