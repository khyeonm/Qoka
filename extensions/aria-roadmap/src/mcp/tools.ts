...
export function buildTools(
	...
	return [
		...
		},
		{
			...
			description: 'Signal that the roadmap is structurally complete. Optional, purely a marker - the roadmap already saves AUTOMATICALLY on every edit, so there is nothing to enable and no Save button. Call at most once, only after the user confirms the roadmap looks done. Do NOT tell the user to save. When the roadmap is complete (including when the user says something like "let\'s finish the roadmap here"), you MUST complete the hand-off so the plan moves onto the Overview and To-do: call add_tasks (the qoka-overview tool) to break the plan into research / work-unit ACTION items, then call open_overview (also qoka-overview) so the plan lands on the Notebook tab, and tell the user the roadmap is on the Notebook tab with the To-do list placed below it. This finalize marker does NOT replace that hand-off.',
			inputSchema: { type: 'object', properties: {}, additionalProperties: false },
			handler: async () => {
				setFinalized(true);
				notify();
