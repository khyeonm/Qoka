...
export async function registerWithClaudeCode(port: number): Promise<RegistrationResult> {
	console.log(`[aria-roadmap] registerWithClaudeCode(port=${port}) starting`);
	const claude = await resolveClaudeBinary();
	if (!claude) {
		console.error('[aria-roadmap] Claude CLI not found in PATH or candidate locations');
		return { ok: false, changed: false, message: 'Claude CLI not found on PATH or known install locations.' };
	}
	console.log(`[aria-roadmap] Claude CLI resolved to: ${claude}`);
	...
	if (existingPort === port) {
		console.log(`[aria-roadmap] Claude Code already registered on port ${port}; skipping re-registration`);
	...
	if (!addResult.ok && /already exists/i.test(addResult.stderr)) {
		// Race condition: autopipe / paper-search / aria-roadmap all touch
	...
	if (!addResult.ok) {
		console.error(`[aria-roadmap] claude mcp add failed:`, addResult.stderr);
	...
	try {
		...
		console.warn(`[aria-roadmap] claude mcp list verification failed:`, e);
	}
