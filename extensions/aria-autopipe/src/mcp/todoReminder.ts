...
import { CallToolResult } from './tools/types';

/**
 * "Todo completion" trigger. When a tool that PRODUCED a deliverable returns a
 * successful result, we append the project's open To-do list to that result so
 * the AI - which just did the work and knows what it was - can propose marking a
 * matching item complete (propose_task_completion, then the user confirms). The
 * code never guesses which task was finished; it only surfaces the candidates at
 * the right moment. The AI does the semantic match.
 *
 * This is a forcing function that does NOT rely on the AI polling get_tasks on
 * its own (which it never did): the reminder rides the tool result the AI reads
 * immediately after finishing a step.
 */

/** Tool names that mean "a deliverable was produced" - matched by leading verb so
 *  new tools are covered without editing a list. Query/setup tools (get_/list_/
 *  check_/read_/search_/set_/open_/start_/prepare_...) are intentionally excluded. */
const PRODUCTIVE_VERB = /^(run|execute|save|create|export|publish|build)_/;

/** Don't re-inject the list within this window, so a burst of tools (run_code then
 *  save_results) nudges once, not every call. */
 ...
 * so the AI can propose completing a matching item. Otherwise returns the result
 * unchanged. Never throws.
 */
export function maybeAppendTodoReminder(toolName: string, result: CallToolResult): CallToolResult {
	try {
		...
		const reminder =
			...
			+ 'If the work you just completed finishes one of these, call propose_task_completion with its id '
			+ '(this shows an Accept/Reject badge in the Project Overview tab), then ask the user to confirm before '
			+ 'set_task_done. If none apply, ignore this.';
