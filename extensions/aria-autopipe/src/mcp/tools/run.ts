...
export const RUN_MCP_INSTRUCTIONS = [
	...
	'',
	'Routing - ASK FIRST BY DEFAULT: when the user asks to run/execute code (실행/돌려/run) and has NOT clearly said whether they want a quick run or a reproducible pipeline, you MUST ASK before running anything: "간단하게 바로 실행할까요(run_code), 아니면 autopipe 파이프라인으로 만들까요?" (Run it quickly now, or build an autopipe pipeline?). Do NOT silently pick one.',
	...
	'Only SKIP the question when the user already made the intent clear (e.g. "그냥 빨리 돌려줘" / "just run this quickly" -> run_code; "파이프라인으로 만들어줘" / "build a pipeline" -> execute_pipeline). Never fall back to the terminal.',
	'',
