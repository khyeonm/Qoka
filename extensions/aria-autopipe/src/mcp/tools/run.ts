				...
				if (savedTo) {
					...
					if (skipped.length) {
						lines.push(`These are too large to copy back automatically (over ${humanSize(MAX_COPY_BYTES)}) and are still on the server: ${skipped.join(', ')}.`
							...
							+ ` If they say yes, use download_results (qoka-run MCP) with the run directory ${resolvedDir || 'reported above'}; it may take a while for a large file.`);
					}
...
export const RUN_MCP_INSTRUCTIONS = [
	...
	'Sequence, every time: (1) call get_workspace_info (qoka-environment MCP) to confirm the ACTIVE run connection is reachable - if not, call start_server then re-check; (2) then run_code here (quick) or execute_pipeline on autopipe (pipeline).',
	...
	'Do NOT default to building an autopipe pipeline: a pipeline needs a Snakefile/config/Dockerfile and is MUCH heavier to set up than run_code. Having used autopipe before in THIS project is NOT a reason to assume a pipeline this time - that is a common mistake; ASK instead of assuming. The routing choice is PER-REQUEST: when the user asks to run a DIFFERENT or NEW analysis - even right after an autopipe run in the SAME session - re-ASK (run_code / pipeline / notebook), never continue the previous mode by inertia.',
	...
	'- Step-by-step, inspect-as-you-go, or the user says "노트북"/"notebook"/"cell by cell" -> use create_notebook (qoka-run MCP) to AUTHOR a .ipynb; the user runs the cells with the "Qoka Run Environment" kernel. Do NOT run notebook cells yourself.',
