...
export const CONFIGURE_INPUT_TOOLS: ToolDefinition[] = [
	{
		...
		description: 'Open a Qoka tab where the USER fills in a pipeline\'s config values and picks its input data files, then clicks "Save and run" to start the pipeline. Use this AFTER download_pipeline, or BEFORE running a pipeline, so the user sets real inputs instead of running with the config\'s placeholder defaults. `pipeline` is the pipeline name (its folder name, i.e. the part after "autopipe-" in the image name). Provide `descriptions` - a map of each config key to a short one-line help string you write (from reading the Snakefile/config/scripts) that is shown under that field. Write these descriptions in ENGLISH (the tab UI is English), and provide one for EVERY editable key so no field falls back to a raw config comment. IMPORTANT: the tab\'s "Save and run" button stages the input data, writes config.yaml, and STARTS the run itself - so after calling this do NOT call execute_pipeline and do NOT poll check_status. Tell the user (in their language) that the input tab is open, to fill it in, pick input files, and click "Save and run", then to tell you once they have. When they confirm, use list_running_pipelines / check_status to report progress.',
		inputSchema: {
			type: 'object',
			properties: {
