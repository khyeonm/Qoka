...
/**
 * Copies the node-pty files from Aria's installation into a @github/copilot location.
 *
 ...
 * Code's app tree). For built-in installs `require('node-pty')` walks up into Aria's
 * own `node_modules` and this shim is a no-op (the `shims.txt` placeholder skips it). For
 * marketplace installs we copy Aria's pre-built `pty.node` into the SDK's expected
 ...
 * @param vscodeAppRoot Aria's installation path (where node-pty is located)
 */
