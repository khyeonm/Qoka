...
/**
 * Convert a JSONL debug log entry into a Aria debug panel event.
 ...
 *   entries because Aria core is already displaying them for live sessions.
...
export function debugLogEntryToDebugEvent(entry: IDebugLogEntry, skipCoreEvents = true): vscode.ChatDebugEvent | undefined {
	switch (entry.type) {
		...
		case 'discovery':
			...
			// by Aria core via onDidReceiveChatDebugEvent for live sessions.
		...
		case 'generic':
			...
			// by Aria core directly for live sessions.
 ...
 * spanId collisions from OTel counter resets across Aria restarts.
 */
