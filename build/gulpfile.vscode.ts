...
const vscodeResourceIncludes = [
	...
	// Qoka - view icons + webview bundles (e.g. ariaNotes' notesEditor.css/.js)
	...
	'out-build/vs/workbench/contrib/aria*/browser/media/*.{svg,png,css,js}',
...
function packageTask(platform: string, arch: string, sourceFolderName: string, destinationFolderName: string, _opts?: { stats?: boolean }) {
	...
	const task = () => {
		...
		if (platform === 'win32') {
			...
			// Only build the APPX package + Explorer context-menu manifest when the
			// product actually ships them (win32ContextMenu / appx assets). Qoka is a
			// stable build but distributes via Inno (.exe) only, without those assets,
			// so this stable-gated block would otherwise crash on product.win32ContextMenu.
