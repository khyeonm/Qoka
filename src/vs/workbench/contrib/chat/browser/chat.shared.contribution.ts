	...
	properties: {
		...
		[mcpAccessConfig]: {
			...
			enumDescriptions: [
				...
				nls.localize('chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that Qoka is connected to."),
			...
			policy: {
				...
				localization: {
					...
					enumDescriptions: [
						...
						{
							key: 'chat.mcp.access.registry', value: nls.localize('chat.mcp.access.registry', "Allows access to MCP servers installed from the registry that Qoka is connected to."),
						},
						{
		...
		[ChatConfiguration.ExtraMarketplaces]: {
			...
			// JSON Schema's `type` keyword (equivalent to `type: 'string'`) to trigger Qoka's
